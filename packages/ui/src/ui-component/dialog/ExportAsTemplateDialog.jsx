...
const ExportAsTemplateDialog = ({ show, dialogProps, onCancel }) => {
    ...
    const onConfirm = () => {
        if (name.trim() === '') {
            enqueueSnackbar({
                message: t('exportTemplate.nameRequired'),
                options: {
    ...
    useEffect(() => {
        if (saveCustomTemplateApi.data) {
            enqueueSnackbar({
                message: t('exportTemplate.saveSuccess'),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'success',
                    persist: true,
    ...
    useEffect(() => {
        if (saveCustomTemplateApi.error) {
            enqueueSnackbar({
                message: t('exportTemplate.saveFailed'),
                options: {
                    key: new Date().getTime() + Math.random(),
                    variant: 'error',
                    persist: true,
    ...
    const component = show ? (
        ...
        >
            <DialogTitle sx={{ fontSize: '1rem' }} id='alert-dialog-title'>
                {dialogProps.title || t('exportTemplate.title')}
            </DialogTitle>
            <DialogContent>
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>
                            {t('exportTemplate.name')}
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('exportTemplate.description')}</Typography>
                        <OutlinedInput
                            id={'description'}
                            type={'string'}
                            fullWidth
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('exportTemplate.badge')}</Typography>
                        <OutlinedInput
                            id={'badge'}
                            type={'string'}
                            fullWidth
                ...
                <Box sx={{ pt: 2, pb: 2 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                        <Typography sx={{ mb: 1 }}>{t('exportTemplate.usecases')}</Typography>
                        {usecases.length > 0 && (
            ...
            <DialogActions>
                ...
                <StyledButton disabled={dialogProps.disabled} variant='contained' onClick={onConfirm}>
                    {dialogProps.confirmButtonName || t('exportTemplate.saveTemplate')}
                </StyledButton>
            </DialogActions>
        </Dialog>
    ) : null
